...
RSpec.xdescribe Sources::Satellite::Operations::Worker do
  describe "#run" do
    ...
    before do
      allow(Sources::Satellite::Connection).to receive_messages(:start_receptor_client => nil,
      ...
      Sources::Satellite::MessagingClient.class_variable_set(:@@default, nil)
    end
    ...
    it "calls subscribe_topic on the right queue" do
      ...
      expect(Sources::Satellite::Operations::Processor)
    ...
    it "starts and stops clients" do
      ...
      expect(Sources::Satellite::Connection).to receive(:start_receptor_client)
      expect(Sources::Satellite::Connection).to receive(:stop_receptor_client)
    ...
    context ".metrics" do
      it "records successful operation" do
        ...
        allow(Sources::Satellite::Operations::Processor).to receive(:process!).and_return(result)
      ...
      it "records exception" do
        ...
        allow(Sources::Satellite::Operations::Processor).to receive(:process!).and_raise("Test Exception!")
        expect(metrics).to receive(:record_operation).with(operation, :status => result)
